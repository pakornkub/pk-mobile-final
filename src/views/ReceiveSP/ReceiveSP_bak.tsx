...
import { Box, Input, Select, Icon, VStack, Button, useToast, FormControl } from 'native-base';
...
const ReceiveSP: React.FC = () => {
  ...
  const { isLoading: orderIsLoading, isFetching, isError, data: orderData, status, error } = useReceiveSP();
              ...
              <FormControl isRequired isInvalid={'Rec_ID' in errors}>
                <Controller
                  ...
                  render={({ field: { onChange, value } }) => (
                    <Select width={'100%'} placeholder="RECEIVE SP ORDER NO." selectedValue={value} onValueChange={onChange}>
                      {orderData?.data?.data?.map((value: any) => {
                        return <Select.Item key={value.Rec_ID} shadow={2} label={value.Rec_NO} value={value.Rec_ID.toString()} />;
                      })}
                    </Select>
                  )}
                  name="Rec_ID"
                  ...
                  defaultValue=""
                />
                <FormControl.ErrorMessage>{errors.Rec_ID?.message}</FormControl.ErrorMessage>
              </FormControl>
                ...
                <Controller
                  ...
                  render={({ field: { value } }) => (
                    <Input
                      ...
                      InputRightElement={<Icon size={25} as={<MaterialIcons name="qr-code-scanner" />} onPress={() => setCamera(true)} />}
                      variant="underlined"
                      p={2}
                      placeholder="SCAN QR"
                  ...
                  defaultValue=""
                />
                <FormControl.ErrorMessage>{errors.QR_NO?.message}</FormControl.ErrorMessage>
              </FormControl>
              <ScrollView
                keyboardShouldPersistTaps="handled"
                style={{ height: '50%' }}
                refreshControl={<RefreshControl refreshing={itemIsLoading} onRefresh={() => itemRefetch()} />}
              >
