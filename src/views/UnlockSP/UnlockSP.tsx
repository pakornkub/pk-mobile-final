...
import { useUnlockSP, useUpdateUnlockSP, useExecUnlockSPTag } from '../../hooks/useUnlockSP';
...
const UnlockSP: React.FC = () => {
  ...
  const { isLoading: orderIsLoading, isFetching, isError, data: orderData, refetch: orderRefetch, status, error } = useUnlockSP();
  ...
  const { isLoading: tagIsLoading, isError: tagIsError, status: tagStatus, error: tagError, mutate: tagMutate, data: tagData } = useExecUnlockSPTag();
  ...
  useEffect(() => {
    if (tagStatus === 'success') {
      toast.show({
        render: () => <AppAlert text={tagData?.data?.message || 'success'} type="success" />,
        placement: 'top',
        duration: 2000,
      });
                    ...
                    {itemData?.data?.data?.map((value: any, key: number) => {
                      return (
                        <DataTable.Row key={key}>
                          ...
                          <DataTable.Cell style={styles.table_title_52}>{value.SP}</DataTable.Cell>
